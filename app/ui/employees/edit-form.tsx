...
export default function EditForm
  ...
  const [spouseDateOfBirth, onChange1] = useState<Value>(new Date(employee.spouseDateOfBirth));


  const handleAddChild = () => {
    setChildren([...children, { childName: '', childGender: 'Male', dateOfBirth: '', childCnic: '' }]);
  };
  ...
  const handleChildGenderChange = (e: any, index: any) => {
    const newChildren = [...children];
    newChildren[index].childGender = e.target.value;
    setChildren(newChildren);
  };
  ...
  const handleChildCnicChange = (e: any, index: any) => {
    const newChildren = [...children];
    newChildren[index].childCnic = e.target.value;
    setChildren(newChildren);
  };
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              ...
              <Input id="mobileNumber" name="mobileNumber" type="text" placeholder="Enter mobile number" defaultValue={employee.mobileNumber} />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="landlineNumber">Landline Number</Label>
              <Input id="landlineNumber" name="landlineNumber" type="text" placeholder="Enter landline number" defaultValue={employee.landlineNumber} />
            </div>

            <div className="grid gap-2 sm:col-span-2">
              <Label htmlFor="currentAddress">Current Address</Label>
              <Textarea id="currentAddress" name="currentAddress" placeholder="Enter current address" defaultValue={employee.currentAddress} />
            </div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              ...
              <Input id="emergencyContactNumber" name="emergencyContactNumber" type="text" placeholder="Enter emergency contact number" defaultValue={employee.emergencyContactNumber} />
            </div>
          </CardContent>
              ...
              {isMarried === 'Yes' && (
                <>
                  ...
                  <div className="grid gap-2 col-span-2 sm:col-span-1">
                    ...
                    <Select name="spouseGender" defaultValue={employee.spouseGender}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select gender" />
                      </SelectTrigger>
                      <SelectContent>
                ...
                children.map((child, index) => {
                  return (
                    <React.Fragment key={`child${index}`}>
                      ...
                      <div key={`childGender${index}`} className="grid gap-2 col-span-2 sm:col-span-1">
                        <Label htmlFor={`childGender${index}`}>Gender</Label>
                        <Select name={`childGender${index}`} defaultValue={child.childGender}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select gender" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem onChange={(e) => handleChildGenderChange(e, index)} value="Male">Male</SelectItem>
                            <SelectItem onChange={(e) => handleChildDateOfBirthChange(e, index)} value="Female">Female</SelectItem>
                            {/* <SelectItem value="other">Other</SelectItem> */}
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      <div key={`childCnic${index}`} className="grid gap-2 col-span-2 sm:col-span-1">
                        <Label htmlFor={`childCnic${index}`}>B-Form/CNIC</Label>
                        <Input id={`childCnic${index}`} name={`childCnic${index}`} placeholder="Enter child cnic" value={child.childCnic} required onChange={(e) => handleChildCnicChange(e, index)} />
                      </div>
                    </React.Fragment>
                  )
                })
